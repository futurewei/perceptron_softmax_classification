...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
        i = 0
        for para in param_vars:
            updates.append((para, para-self.learning_rate*grad_tensors[i]))
            i = i + 1
        return updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for var in param_vars:
            v = (self.momentum * vel_vars[j]) - (self.learning_rate * grad_tensors[j])
            updatedWeight = var+v
            updates.append((var, updatedWeight))   # updated wt+1
            updates.append((vel_vars[j], v))  # updated vt+1
            j = j+1
        return updates
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        trainGenerator = MinibatchIndefinitelyGenerator(train_data ,1, self.shuffle)
        validateGenerator = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            train_input = trainGenerator.next()
            train_loss = session.run([loss_tensor, update_ops], feed_dict={placeholders[0]: train_input[0], 
                placeholders[1]: train_input[1]})
            train_loss = train_loss[0]
            valid_input = validateGenerator.next()
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: valid_input[0], 
                placeholders[1]: valid_input[1]})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        trainGenerator = MinibatchIndefinitelyGenerator(train_data ,self.batch_size, self.shuffle)
        validateGenerator = MinibatchIndefinitelyGenerator(val_data, self.batch_size, self.shuffle)
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            train_input = trainGenerator.next()
            train_loss = session.run([loss_tensor, update_ops], feed_dict={placeholders[0]: train_input[0], 
