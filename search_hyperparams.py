...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        "*** YOUR CODE HERE ***"
        stoGraSolver = solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)
        stoGraSolver.solve(train_data, train_labels, val_data, val_labels, model)
        accurate = model.accuracy(val_data, val_labels)
        if accurate > best_accuracy:
            best_accuracy = accurate
            best_model = model
            best_hyperparams = best_index
