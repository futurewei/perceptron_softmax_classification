...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                "*** YOUR CODE HERE ***"
                predicted = self.classify(input_train_data[i])
                if predicted == label_train_data[i]:
                    continue
                else:
                    self.weights[label_train_data[i]] = self.weights[label_train_data[i]] + input_train_data[i]
                    self.weights[predicted] = self.weights[predicted] - input_train_data[i]
    ...
    def find_high_weight_features(self, label):
        ...
        for w in weight:
            temp.append((weight[w], w))

        temp = sorted(temp)
        print (temp)
        for i in range(len(temp)-1, len(temp)-101, -1):
            best100Features.append(temp[i][1])


        return best100Features

def dot_product(weights, feature):
        result = 0
