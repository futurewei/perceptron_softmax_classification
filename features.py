...
def enhancedFeatureExtractor(datum):
    ...
    for row in range(len(datum)):
        for col in range(len(datum[0])):
            if datum[row][col] <= 0:
                whiteSpaceCount +=1
            if row < 9 and datum[row][col]>0:
                sect1_total += 1
            elif row >=9 and row < 18  and datum[row][col]>0:
                sect2_total += 1
            elif row >=18 and row < 28  and datum[row][col]>0:
                sect3_total += 1
    #normalized:
    for row in range(len(datum)):
        for col in range(len(datum[0])):
    ...
    # used to tell how many straight line.
    connected = numConnected(datum, 28, 28)
    ...
    for i in range(28):
        if medians[i] > 20:
            Long+=1
    '''
    # ratio within sections:
    section1=72
    section2=72
    section3=80
    return np.append(features, [connected, sect1_total/section1, sect2_total/section2, sect3_total/section3])


def numConnected(image, m, n):
    total = 0
    for i in range(m):
        for j in range(n):
            if image[i][j] != 0:
                ...
                while len(stack) != 0:
                    [p,q] = stack.pop()
                    if p >= 1 and image[p-1][q]== 1:
                        stack.append([p-1,q])
                    if p < m -1 and image[p+1][q]== 1:
                        stack.append([p+1,q])
                    if q >= 1 and image[p][q-1]== 1:
                        stack.append([p,q-1])
                    if q < n - 1 and image[p][q + 1]== 1:
                        stack.append([p,q+1])
                    image[p][q] = 0
    return total
